#!/bin/bash
# Coverage Script for Codex Plus
...
SOURCE_DIR="$PROJECT_ROOT/src/codex_plus"

# Coverage output directory
COVERAGE_DIR="/tmp/codex_plus/coverage"
...
print_status "🧪 Running coverage analysis for Codex Plus..."
...
# Use pytest if available, otherwise run individual test files
if command -v pytest >/dev/null 2>&1 && [ -f "$PROJECT_ROOT/pytest.ini" ]; then
    ...
    if coverage run --source=src/codex_plus -m pytest tests/ -v; then
        ...
        # For simplicity, mark all as failed if pytest fails
