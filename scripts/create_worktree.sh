#!/bin/bash
# This script creates a new Git worktree with a specified name for Codex Plus development.
...
if [ -n "$1" ]; then
    ...
    read -p "Enter the desired base name for the new worktree (e.g., 'feature-xyz'): " RAW_WORKTREE_NAME
fi
...
if git worktree add "$WORKTREE_PATH" -b "$WORKTREE_NAME"; then
    ...
    # Copy critical files for development
    if [ -f "../proxy.sh" ]; then
        cp "../proxy.sh" . 2>/dev/null || true
        info_message "Copied proxy.sh for development"
    fi
    ...
    echo "  git worktree remove      # Remove a worktree"
    echo "  git branch -d            # Delete the branch when done"
    echo "  ./proxy.sh enable        # Start Codex Plus proxy"
else
